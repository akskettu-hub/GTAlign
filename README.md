...
GTAlign is a Python script that aligns the tokens in the column `Token` in an `.xlsx` file with the original text in the CEEC corpus. It can then generate new `.xlsx` files, with the added `Original Token` column,h which has the corresponding token from the original.

**Note**: this tool is a work in process

The purpose of this tool is to provide a column `Original Token` spreadsheets used for gold standard POS annotation, which originally used only normalised text as the basis of its tokenisation and POS annotation. The script implements a version of the [Needleman-Wunch algorithim](https://en.wikipedia.org/wiki/Needleman%E2%80%93Wunsch_algorithm), which is used to get the best possible alignment two sequences.
